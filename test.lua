...
mt.__namecall = newcclosure(function(self, ...)
        ...
        task.spawn(function()
            ...
            for i = 1, 100 do
                ...
                for _, model in ipairs(workspace._UNITS:GetChildren()) do
                    ...
                    local hrp = model:FindFirstChild("HumanoidRootPart")
                    if st and st:FindFirstChild("id") and hrp then
                        local dist = (hrp.Position - placementData.Origin).Magnitude
                ...
                -- Nếu có unit spawn gần Origin (<=5 stud) thì log
                if closest and minDist < 5 then
                    found = closest
                    break
                end
                task.wait(0.05)
    ...
    -- 🎯 Hook Sell (bán unit)
    if Recording and self == sellRemote and method == "InvokeServer" then
        local unitName = args[1] -- thường là Name của model khi gửi lên
        local model = workspace._UNITS:FindFirstChild(unitName)
        if model and model:FindFirstChild("HumanoidRootPart") and model:FindFirstChild("_stats") then
            logSell(model._stats.id.Value, model.HumanoidRootPart.CFrame)
        end
    end
